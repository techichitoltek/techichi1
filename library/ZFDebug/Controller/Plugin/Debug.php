...
class ZFDebug_Controller_Plugin_Debug extends Zend_Controller_Plugin_Abstract
{
    ...
    public function dispatchLoopShutdown()
    {
        ...
        foreach ($this->_plugins as $plugin)
        {
            ...
            {
                $debug_session = new Zend_Session_Namespace(App_Debug_Debug::DEBUG_NAMESPACE);
        ...
        foreach ($this->_plugins as $plugin)
        {
            ...
            if (strtolower($plugin->getTab()) == 'debug' && $plugin->getPanel() != '')
            {
                ...
                Zend_Session::namespaceUnset(App_Debug_Debug::DEBUG_NAMESPACE);
            }
            else
            {
                $style = '';
    ...
    {
        $panel = '<h4>Debug v'.$this->_version.'</h4>';
