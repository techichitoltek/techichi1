...
class App_View_Helper_AutoloadResources extends Zend_View_Helper_Abstract{
    ...
    public function AutoloadResources(){
        ...
        if(file_exists($file)){
            if(CURRENT_MODULE == 'backoffice'){
                $file = $this->view->designUrl() . '/js/autoload/' . $controllerName . '/' . strtolower(App_Inflector::camelCaseToDash($actionName)) . '.js';
                $this->view->headScript()->offsetSetFile(2, $file);
            }else{
                $file = $this->view->designUrl() . '/js/autoload/' . strtolower(App_Inflector::camelCaseToDash($controllerName)) . '.js';
