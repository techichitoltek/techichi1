 ...
 * @category   App
 * @package    App_Tool
 * @copyright  Copyright RCWEB
 */
class App_Tool_Project_Provider_ZfsController extends Zend_Tool_Project_Provider_Controller
{
	
    /**
     * createResource will create the zfsControllerFile resource at the appropriate location in the
     * profile.
     *
     ...
     */
    public static function createResource(Zend_Tool_Project_Profile $profile, $controllerName, $moduleName = 'backoffice')
    {
    ...
    public static function hasResource(Zend_Tool_Project_Profile $profile, $controllerName, $moduleName = null)
    {
        ...
        return (($controllersDirectory->search(array('zfsControllerFile' => array('controllerName' => $controllerName)))) instanceof Zend_Tool_Project_Profile_Resource);
    }
     ...
     */
    public function create($name, $indexActionIncluded = true, $module = 'backoffice')
    {
        ...
        try {
            ...
            if ($indexActionIncluded) {
                $indexActionResource = App_Tool_Project_Provider_ZfsAction::createResource($this->_loadedProfile, 'index', $name, $module);
                $indexActionViewResource = App_Tool_Project_Provider_ZfsView::createResource($this->_loadedProfile, 'index', $name, $module);
            }
            if ($testingEnabled) {
